...
struct FaceDetectTfConfig : public TfConfig
{
...
class FaceDetectTfStage : public TfStage
{
public:
	FaceDetectTfStage(RPiCamApp *app) : TfStage(app, INPUT_WIDTH, INPUT_HEIGHT)
	{
		config_ = std::make_unique<FaceDetectTfConfig>();
	}
	char const *Name() const override { return NAME; }

protected:
	FaceDetectTfConfig *config() const { return static_cast<FaceDetectTfConfig *>(config_.get()); }
...
void FaceDetectTfStage::readExtras(boost::property_tree::ptree const &params)
{
...
void FaceDetectTfStage::decodeRegression(const float *reg, std::vector<std::vector<float>> &decoded_boxes)
{
...
void FaceDetectTfStage::applyNMS(const std::vector<std::vector<float>> &boxes, const std::vector<float> &scores,
								 std::vector<int> &selected_indices)
{
...
void FaceDetectTfStage::interpretOutputs()
{
...
void FaceDetectTfStage::applyResults(CompletedRequestPtr &completed_request)
{
...
{
	return new FaceDetectTfStage(app);
}
